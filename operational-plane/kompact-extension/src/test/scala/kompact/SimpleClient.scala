...
final case class ClientHandler(testActorPath: String) extends ChannelInboundHandlerAdapter with LazyLogging with TestSettings {
  ...
  override def channelActive(ctx: ChannelHandlerContext): Unit = {
    ...
    val src = KompactAkkaPath("kompact", "127.0.0.1", 2020)
    val dst = KompactAkkaPath(testActorPath, "127.0.0.1", 1337)
    val s = KompactRegistration(testActorPath, Some(src), Some(dst))
    val reg = KompactAkkaEnvelope(Some(src), Some(dst), msg = Some(KompactAkkaMsg().withKompactRegistration(s)))
